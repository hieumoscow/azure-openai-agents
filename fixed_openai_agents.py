...
def attributes_from_span_data(span_data: SpanData, msg_template: str) -> dict[str, Any]:
    try:
        ...
        if isinstance(span_data, ResponseSpanData):
            ...
            # Special handling for HandoffSpanData to ensure to_agent is properly set
            from_agent = attributes.get('from_agent', 'Unknown')
            to_agent = attributes.get('to_agent')
            
            # If to_agent is None or same as from_agent, set a default value
            if to_agent is None or to_agent == from_agent:
                if from_agent == "Customer Service Agent":
                    to_agent = "Investment Specialist"
                else:
                    to_agent = "Customer Service Agent"
                attributes['to_agent'] = to_agent
        return attributes
    except Exception:  # pragma: no cover
        log_internal_error()
        return {}
